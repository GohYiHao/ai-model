...
with gr.Blocks() as demo:
    ...
    with gr.Row():
                    ...
                    if property_info.get("type", {}) == "integer":
                        ...
                        if  property_info.get('format','') == 'uri':
                            if 'image' in property_info.get('title', '').lower():
                                if value :
                                        inputs.insert(order, gr.Image(label=property_info.get('title', ''), value=value, type="filepath"))
                                else :
                                        inputs.insert(order, gr.Image(label=property_info.get('title', ''), type="filepath"))
        ...
        with gr.Column():
            ...
            outputs = []
            outputs.append(gr.Image(value='https://pbxt.replicate.delivery/YXbcLudoHBIYHV6L0HbcTx5iRzLFMwygLr3vhGpZI35caXbE/out-0.png'))
           ...
           if input4:
                body = {
                        ...
                        "input": {
                            ...
                            "prompt-strength": input11,
                            "seed":input12,
                            "refine":input13,
                            "high_noise_frac":input14,
                            "refine_steps":input15,
                            "apply_watermark": input16,
                            "lora_scale":input17,
    ...
    def cancel_process(input1,input2,input3,input4,input5,input6,input7, input8, input9,input10,input11,input12,input13,input14,input15,input16,input17,input18, input19):
           global cancel_url
           cancel_url = "123"
           return gr.Image(value='https://pbxt.replicate.delivery/YXbcLudoHBIYHV6L0HbcTx5iRzLFMwygLr3vhGpZI35caXbE/out-0.png'), gr.Image(visible=False),gr.Image(visible=False),gr.Image(visible=False)
